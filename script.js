...
function renderTasks() {
  ...
  if (tasks.length === 0) {
    tasksList.innerHTML =
      '<div class="empty-state">Belum ada tugas. Tambahkan tugas pertama Anda!</div>';
    return;
  }
    ...
    .map((task) => {
      ...
      const warningHTML =
        warning && !task.completed
          ? `<span class="warning ${warning.color}">${warning.text}</span>`
          : "";
...
document
  .getElementById("btnToggleSettings")
  .addEventListener("click", toggleSettings);
document
  .getElementById("btnToggleDark")
